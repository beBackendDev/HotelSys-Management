# **JAVA SPRING**
**Specification:** Trước tiên để biết về Specification trongg Spring thì ta cần biết thêm về JPA( stand for `Java Persistence API`) là một tiêu chuẩn kĩ thuật chính thức của Java về việc lưu trữ và thao tác dữ liệu trong CSDL quan hệ một cách đơn giản và hiệu quả. Trong JPQ, `Specification` là một API mạnh mẽ giúp xây dựng các truy vấn cơ sở dữ liệu một cách linh hoạt và hiệu quả.
